...
func Bloger(c echo.Context) error {
	...
	data, _ := connection.Conn.Query(context.Background(), "SELECT coba5.id, tb_user.name, coba5.nama, coba5.stardate, coba5.enddate, coba5.durasi, coba5.deskripsi, coba5.nodejs, coba5.react, coba5.nextjs, coba5.typescript FROM coba5 LEFT JOIN tb_user ON coba5.authdor_id = tb_user.id")
	...
	for data.Next() {
		...
		errs := data.Scan(&each.Id, &authdorSementara, &each.Nama, &each.PostDate, &each.EndDate, &each.Durasi, &each.Deskripsi, &each.NodeJs, &each.ReactJS, &each.NextJs, &each.TypeScript)
